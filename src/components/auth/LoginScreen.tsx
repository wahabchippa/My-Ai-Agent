...
export function LoginScreen({ onSuccess, onSwitchToSignup, onSwitchToForgotPassword }: LoginScreenProps) {
  ...
  // ── Pick up auth_error / auth_success from URL (OAuth redirect) ──
  useEffect(() => {
    ...
    // OAuth success → bootstrap user from cookie and redirect
    const authSuccess = url.searchParams.get("auth_success");
    if (authSuccess) {
      url.searchParams.delete("auth_success");
      window.history.replaceState({}, "", url.pathname);

      // Try reading the bootstrap cookie
      ...
      if (bootstrapCookie) {
        try {
          const userData = JSON.parse(decodeURIComponent(bootstrapCookie.split("=").slice(1).join("=")));
  ...
  const handleOAuthLogin = (provider: "google" | "github") => {
    ...
    // Redirect to server-side OAuth initiation endpoint.
    // The server generates a state token, stores it, and redirects
  ...
  return (
    <AuthLayout>
      <div className="rounded-2xl border border-border bg-surface p-8 shadow-lg">
        ...
        {success && (
          <div className="mb-4 rounded-lg bg-success-soft border border-success/20 px-4 py-3 text-sm text-success animate-fade-in">
            {success}
          </div>
        )}
        ...
        <div className="space-y-3 mb-6">
          ...
          >
            {oauthLoading === "google" ? (
              <>
                <div className="h-5 w-5 animate-spin rounded-full border-2 border-text-muted border-t-text" />
